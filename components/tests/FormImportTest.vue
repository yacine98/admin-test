  ...
  methods: {
    ...
    async submitForm() {
      ...
      try {
        ...
        if (total === 0) {
          this.$store.dispatch('toast/getMessage', { type: 'error', text: 'Aucune donnée à importer.' })
          this.progressDialog = false
          this.loading = false
          return
        }

        // Simule une progression ligne par ligne
        for (let i = 0; i < total; i++) {
          await new Promise(resolve => setTimeout(resolve, 10)) // remplace ceci si tu fais un vrai traitement ligne par ligne
          this.progress = Math.round((i / total) * 100)
        }

        // Étape 3 : Appel API
        // const response = await this.$mrvciApi.post('/importDonneetest', {
        //   data: this.listdonneetests
        // })

        this.$store.dispatch('toast/getMessage', { type: 'success', text: response.data.message })
        this.$store.commit('donneetests/initlist', [])
        this.selectXlsx = null
      } catch (error) {
        this.$store.dispatch('toast/getMessage', { type: 'error', text: error.message || 'Échec de l\'importation' })
      } finally {
        this.loading = false
        this.progress = 100
      ...
      reader.onload = (e) => {
        try {
          ...
          for (let i = 1; i < data.length; i++) {
            ...
            // row["id_secteur"] = this.model.secteur?.id;
            // row["id_annee"] = this.listannees.find(annee => annee.libelle == row["annee"].toString())?.id;
            // row["id_categorie_combustible"] = this.listcategories.find(categorie => categorie.code == row["code_activite"].toString())?.id;
            // row["id_combustible"] = this.listcombustibles.find(combustible => combustible.code_combustible == row["code_combustible"].toString())?.id;
            // row["id_user"] = this.model.loggedInUser?.id
            // row["id_structure"] = this.model.loggedInUser?.structure_id;
            // row["status"] = this.model.action?.status?.identifiant;
            // row["state"] = this.model.state;
            list.push(row);
